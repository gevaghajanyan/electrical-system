...
export function ResistorNetworkCalc() {
  const { t } = useTranslation()
  const [mode, setMode] = useState<Mode>('parallel')
  const [values, setValues] = useState<string[]>(['100', '220', '470'])
  ...
  const total = (() => {
    ...
    if (mode === 'series') return nums.reduce((s, r) => s + r, 0)
    const invSum = nums.reduce((s, r) => s + 1 / r, 0)
    return invSum > 0 ? 1 / invSum : 0
  })()

  return (
    <CalcCard>
      <CalcHeader title={t('calc.resistor_net.title')} description={t('calc.resistor_net.description')} />
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Button size="sm" variant="outline" onClick={addRow} className="w-full">
            + {t('calc.resistor_net.addResistor')}
          </Button>
        </div>

        <ResultBig
          label={t('calc.resistor_net.total')}
          value={total < 1000 ? fmt(total, 3) : fmt(total / 1000, 3)}
          unit={total < 1000 ? 'Ω' : 'kΩ'}
        />
      </div>
    </CalcCard>
  )
